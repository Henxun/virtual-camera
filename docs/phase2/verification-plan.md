...
| A2 | (automated) Unit tests pass | `python tools\make.py test` | pytest exits 0 |
| A3 | DLL has version info | right-click DLL → Properties → Details | ProductName = "AK Virtual Camera", FileVersion = "0.2.0.0" |
| A4 | Registration succeeds | `python tools\make.py register` (admin) | regsvr32 returns 0; `akvc status` shows DLL path |
| A5 | Filter Mapper registration | open `graphedt.exe` (admin not required) → Graph → Insert Filters → **Video Capture Sources** | `AK Virtual Camera` listed |
| A6 | Unregister leaves no residue | `python tools\make.py unregister` then `akvc status` | "(not registered)" |
...
| B4 | Shared region is created | `python -c "import mmap; m=mmap.mmap(-1,16*1024*1024,'akvc-frames-v1'); print(m[:4].hex())"` while running | output is `41 4b 56 43` |
...
| C6 | graphedt | Insert filter → render output pin → Run | Video Renderer window shows pattern |
...
A short Markdown report is generated by the matrix runner (planned for Phase 6)
under `tests/matrix/results/phase2-<date>.md`. In Phase 2 we record the result
manually in `docs/phase2/verification-results.md` (created at exit time).
