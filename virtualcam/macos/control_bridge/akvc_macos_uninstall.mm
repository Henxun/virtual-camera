...
static BOOL AKVCUninstallStatusConverged(NSDictionary* payload) {
    ...
    if ([state isEqualToString:@"not_installed"] || [state isEqualToString:@"install_failed"]) {
        return YES;
    }
    if ([payload[@"needs_reboot"] respondsToSelector:@selector(boolValue)] && [payload[@"needs_reboot"] boolValue]) {
        return YES;
    }
...
static NSDictionary* AKVCPollUninstallStatusUntilDeadline(NSDate* deadline, BOOL* converged) {
    NSDictionary* payload = nil;
    BOOL didConverge = NO;
    while ([deadline timeIntervalSinceNow] > 0) {
        payload = AKVCQuerySystemExtensionStatus(AKVCCameraExtensionIdentifier(), 0.5);
        if (payload != nil && AKVCUninstallStatusConverged(payload)) {
            didConverge = YES;
            break;
        }
...
int main(int argc, const char * argv[]) {
    ...
    @autoreleasepool {
        ...
        BOOL launchedHost = AKVCLaunchHostAgent(@[@"--deactivate"], &hostError);

        NSDate* deadline = [NSDate dateWithTimeIntervalSinceNow:3.0];
        BOOL converged = NO;
        NSDictionary* payload = nil;
        if (launchedHost) {
            payload = AKVCPollUninstallStatusUntilDeadline(deadline, &converged);
            if (converged) {
            ...
            (void)AKVCWriteJSONObject(@{
                ...
                @"last_error": effectiveError.localizedDescription
                    ?: @"failed to launch container app deactivation request",
            });
            return 1;
        }
        if (payload == nil) {
        ...
        fallback[@"last_error"] = fallback[@"last_error"] ?: @"timed out waiting for extension deactivation";
