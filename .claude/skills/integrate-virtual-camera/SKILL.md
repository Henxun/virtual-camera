...
description: 将 AK Virtual Camera 集成到外部 PySide6/Python 项目。默认推荐仓库根安装 + akvc.sdk.VirtualCamera；仅在需要自定义 helper/pipeline/runtime 时再降级到底层 API。
---
...
默认推荐：**直接从仓库根安装，然后使用 `akvc.sdk.VirtualCamera`**。
...
Windows 安装包默认携带运行时二进制：

- `akvc_helper.exe`
- `akvc-mf.dll`
- `akvc-dshow.dll`

这条路径适合大多数外部 PySide6 / Python 集成场景；不再要求你手写 `HelperService` / `WindowsShmSink` / `FramePipeline` 组装逻辑。

## 一次性注册与验证

如果你要让 OBS / Zoom / GraphStudioNext 通过 DirectShow 发现设备，仍需先在**管理员 shell**中注册：
...
对安装器来说，直接调用内嵌 Python 的 `-m akvc_cli` 通常比依赖 PATH 里的 `akvc.exe` 更稳。
